...
public class MazeSolver {
    ...
    // Finds the minimum number of moves required to collect all keys and reach the exit point.
    public static int findMinimumMovesToCollectAllKeys(String[] grid) {
        ...
        // Finding the starting point and determining the target keys
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                ...
                if (cell == 'S') {
                    ...
                    // Setting the bit for the exit door
                    targetKeys |= (1 << ('f' - 'a'));
                } else if (cell >= 'a' && cell <= 'f') {
                    // Setting the bit for the key
                    targetKeys |= (1 << (cell - 'a'));
                }
            }
        }
